...
url2 = 'https://jobsearch.api.jobtechdev.se'
url2_for_search = f"{url}/search"
...
def _get_ads2(params2):
    headers = {'accept': 'application/json'}
    response = requests.get(url2_for_search, headers=headers, params=params2)
    response.raise_for_status()  # check for http errors
    return json.loads(response.content.decode('utf8'))
...
def example_search_loop_through_hits(querystr):
    ...
    search_params = {'q': querystr, 'limit': 10}
    json_response = _get_ads(search_params)
...
def example_search_loop_through_hits2(querystr2):
    # limit = 100 is the max number of hits that can be returned.
    # If there are more (which you find with ['total']['value'] in the json response)
    # you have to use offset and multiple requests to get all ads.
    search_params = {'q': querystr2, 'limit': 10}
...
if __name__ == '__main__':
    query = st.text_input("Titel: ")
    query2 = st.text_input("År: ")
    querystr = str(query)
    querystr2 = str(query)
    example_search_loop_through_hits(querystr)
    example_search_loop_through_hits2(querystr)
